          ...
          if (Array.isArray(q) && q.length > 0) {
            if (q.length == 2) {
              ...
              if (typeof fn === "function" && !(fn instanceof RegExp)) {
                ...
                // @ts-expect-error: Due to complexity
                result[q[0]] = match ? fn(match, q[2]) : undefined;
              } else {
              ...
              // @ts-expect-error: Due to complexity
              const replaced = match.replace(q[1], q[2]);
              // @ts-expect-error: Due to complexity
              result[q[0]] = match ? q[3](replaced) : undefined;
            }
          } else {
            // @ts-expect-error: Due to complexity
...
function strMapper(str: string, map: string[]): string | undefined {
  for (const i in map) {
...
type PatternArray = (RegExp[] | (Transformation | Transformation[])[])[];

const regexes: Record<string, PatternArray> = {
  browser: [
    [
